...
<?php 
	...
	{	
		$queryy="SELECT passenger_id FROM passengers WHERE trip_id='".mysqli_real_escape_string($link,$rowx['trip_id'])."'";
		$resulty=mysqli_query($link,$queryy);
		while($rowy=mysqli_fetch_array($resulty))
		{

			$queryz="SELECT * FROM rating WHERE rating_uid='".mysqli_real_escape_string($link,$_SESSION['id'])."' AND rated_uid='".mysqli_real_escape_string($link,$rowy['passenger_id'])."' AND trip_id='".mysqli_real_escape_string($link,$rowx['trip_id'])."'";
			$resultz=mysqli_query($link,$queryz);
			$rowz=mysqli_fetch_array($resultz);

			if(!$rowy['passenger_id'])
				continue;
			...
			{
				$querya="SELECT name FROM user WHERE user_id='".mysqli_real_escape_string($link,$rowy['passenger_id'])."'";
				...
				$rate_url="rate_p.php?trip_id=".$rowx['trip_id']."&user=".$rowy['passenger_id'];
